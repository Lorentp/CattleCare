...
class ExcelManager {
  async createExcelAllCalves(calves) {
    try {
      ...
      sheet
        .cell("I2")
        .value("Este archivo generado por Cattle Care || Contacto: 3534270126");
      ...
      calves.forEach((calf, index) => {
        ...
        if (typeof calf.treatment === "string") {
          ...
          const treatmentItem = calf.treatment[0];
          if (typeof treatmentItem === "string") {
            treatmentName = treatmentItem;
          } else if (
            treatmentItem &&
            typeof treatmentItem === "object" &&
            treatmentItem.title
          ) {
            treatmentName = treatmentItem.title;
          }
        } else {
          treatmentName = "";
        }
  ...
  async createExcelReleasedCalves(calvesReleased) {
    try {
      ...
      sheet
        .cell("K2")
        .value(
          "Los terneros que no tengan un peso inicial se les agrega 35kl que es un valor promedio"
        );
